-- Auto-delete completed tasks after archiving
-- When a task is marked complete, it gets archived to task_history
-- and then soft-deleted from tasks table (removed from active task list)
...
BEGIN
    ...
    IF v_status_complete = TRUE AND (v_old_status_complete IS NULL OR v_old_status_complete = FALSE) THEN
        ...
        -- Soft-delete the task (remove from active task list)
        NEW.deleted_at := NOW();
    END IF;
